...
const team = [
  {
    ...
    portfolio: "https://freelanceconnect.cm/cto/",
  },
  {
    ...
    linkedin: "https://www.linkedin.com/in/alhadji-ali-garba-19a815243/",
  },
  {
...
export default function TeamPage() {
  return <div className="min-h-screen bg-background"><Navigation /><main>
    ...
    <section className="container mx-auto px-6 py-20 lg:px-10"><div className="grid items-center gap-10 lg:grid-cols-[.88fr_1.12fr] lg:gap-16"><div className="relative min-h-[430px] overflow-hidden rounded-2xl border-8 border-white bg-white shadow-xl md:min-h-[560px]"><a href="https://bdconsulatecam.cm/consul/" target="_blank" rel="noreferrer" aria-label="Voir le portfolio du consul HC Nsangou Mbombo Zounedou" className="group absolute inset-0 block"><img src="/team/promoteur.jpeg" alt="HC Nsangou Mbombo Zounedou, promoteur de Global Academy" className="h-full w-full object-cover object-center transition duration-300 group-hover:scale-[1.03]" /></a><div className="pointer-events-none absolute inset-x-0 bottom-0 bg-gradient-to-t from-forest via-forest/75 to-transparent p-7 pt-32"><p className="text-sm font-bold uppercase tracking-[.18em] text-gold">Le promoteur</p><p className="mt-2 font-serif text-3xl font-bold text-white">HC Nsangou Mbombo Zounedou</p><p className="mt-3 inline-flex font-bold text-gold">Voir le portfolio du consul <span aria-hidden="true">↗</span></p></div></div><div><p className="eyebrow">Notre promoteur</p><h2 className="mt-3 font-serif text-4xl font-bold text-forest md:text-5xl">HC Nsangou Mbombo Zounedou</h2><p className="mt-3 text-lg font-semibold text-gold-dark">Promoteur de Global Academy</p><div className="mt-7 border-l-4 border-gold pl-6"><Quote className="h-7 w-7 text-gold-dark" /><p className="mt-4 text-lg leading-8 text-muted-foreground">Porter une ambition qui transforme : offrir à chaque apprenant un cadre solide, des repères justes et des opportunités concrètes pour s&apos;exprimer.</p></div><ul className="mt-8 grid gap-3 text-muted-foreground"><li className="flex gap-3"><Award className="mt-1 h-5 w-5 shrink-0 text-gold-dark" />Honorary Consul of Bangladesh in Cameroon</li><li className="flex gap-3"><Award className="mt-1 h-5 w-5 shrink-0 text-gold-dark" />Logistics Expert And International Trade</li><li className="flex gap-3"><Award className="mt-1 h-5 w-5 shrink-0 text-gold-dark" />Knight of the Order of Valour</li></ul><a href="https://www.linkedin.com/in/zounedou-nsangou-349140270/" target="_blank" rel="noreferrer" className="mt-8 inline-flex rounded-md bg-forest px-5 py-3 font-bold text-white transition hover:bg-forest-light">Linkedin du Promoteur <span aria-hidden="true">↗</span></a></div></div></section>

    <section className="bg-cream"><div className="container mx-auto px-6 py-20 lg:px-10"><div className="max-w-2xl"><p className="eyebrow">L&apos;équipe</p><h2 className="mt-3 font-serif text-4xl font-bold text-forest md:text-5xl">Des expertises réunies autour des apprenants.</h2><p className="mt-5 text-lg leading-8 text-muted-foreground">Une équipe complémentaire qui veille à la qualité de chaque expérience au sein de Global Academy.</p></div><div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">{team.map((member) => <article key={member.role} className="overflow-hidden rounded-xl border border-border bg-white shadow-sm"><div className="aspect-[4/5] overflow-hidden"><ProfileImage src={member.image} alt={member.name ? `Portrait de ${member.name}` : "Portrait du responsable de communication"} /></div><div className="p-6"><p className="text-sm font-bold uppercase tracking-[.15em] text-gold-dark">Global Academy</p><h3 className="mt-3 font-serif text-2xl font-bold text-forest">{member.name && <span className="block text-xl">{member.name}</span>}{member.role}</h3><p className="mt-3 leading-7 text-muted-foreground">{member.expertise}</p><div className="mt-5 flex flex-wrap gap-x-6 gap-y-3">{member.linkedin && <a href={member.linkedin} target="_blank" rel="noreferrer" className="inline-flex font-bold text-forest transition hover:text-gold-dark">Voir le profil LinkedIn <span aria-hidden="true">↗</span></a>}{member.portfolio && <a href={member.portfolio} target="_blank" rel="noreferrer" className="inline-flex font-bold text-forest transition hover:text-gold-dark">Voir le portfolio <span aria-hidden="true">↗</span></a>}</div></div></article>)}</div></div></section>
  </main><Footer /></div>
}
